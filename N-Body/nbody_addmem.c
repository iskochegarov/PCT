...
void calculate_forces(struct particle *p, struct particle *f[], float *m, int n)
{
    ...
    // Суммарная сила будет хранится в первой строке – f[0][i]
    #pragma omp single
    {
        for (int i = 0; i < n; i++)
        {
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        f[0][i].x = f[0][i].y = f[0][i].z = 0; //Is it need initial??
    }
    ...
    #pragma omp parallel
    {
        ...
        {
            calculate_forces(p, f, m, n); // Вычисление сил
            #pragma omp barrier
    ...
    double serial_t = 58.144830;
    double speedup = serial_t/ttotal;
